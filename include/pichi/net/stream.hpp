...
template <typename Buffer, typename InputIt>
size_t writeToBuffer(Buffer &buf, InputIt it, size_t n) {
...
template <typename Signature, typename Initiation, typename CompletionToken,
          typename... Args>
auto asyncInitiate(Initiation &&initiation, CompletionToken &&token,
                   Args &&... args) {
  auto t = std::forward<CompletionToken>(token);
  auto init =
      boost::asio::async_completion<std::decay_t<CompletionToken>, Signature>{
          t};
...
struct IteratorConnectOperator {
  IteratorConnectOperator(Stream &s, Iterator it, ConnectHandler const &h)
      : s_{s}, it_{it}, h_{h} {}
...
template <typename T>
inline constexpr bool IsTlsStreamV = IsTlsStream<T>::value;
...
public:
  ...
  TlsStream(Context &&ctx, Args &&... args)
      : ctx_{std::move(ctx)}, stream_{Socket{std::forward<Args>(args)...},
                                      ctx_} {}
  ...
  auto async_connect(typename Socket::endpoint_type const &peer,
                     ConnectHandler &&handler) {
    return stream_.next_layer().async_connect(
        peer, std::forward<ConnectHandler>(handler));
  }

  template <typename HandshakeHandler>
  auto async_handshake(boost::asio::ssl::stream_base::handshake_type t,
  ...
  auto async_read_some(MutableBufferSequence const &buf,
                       ReadHandler &&handler) {
    return stream_.async_read_some(buf, std::forward<ReadHandler>(handler));
  }

  template <typename ConstBufferSequence, typename WriteHandler>
  auto async_write_some(ConstBufferSequence const &buf,
                        WriteHandler &&handler) {
    static_assert(std::is_invocable_v<std::decay_t<WriteHandler>,
                                      ErrorCode const &, size_t>);
...
template <typename T>
struct IsTlsStream<TlsStream<T>> : public std::true_type {};
...
private:
  ...
  static auto read(Buffer &buf, OutputIt it, size_t n) {
  ...
  static auto write(Buffer &buf, InputIt it, size_t n) {
    std::copy_n(it, n, boost::asio::buffers_begin(buf.prepare(n)));
    buf.commit(n);
    return n;
  }
...
public:
  ...
  auto read(MutableBufferSequence const &buf) {
    return read(rBuf_, boost::asio::buffers_begin(buf),
                boost::asio::buffer_size(buf));
  }

  template <typename ConstBufferSequence>
  auto write(ConstBufferSequence const &buf) {
    return write(wBuf_, boost::asio::buffers_begin(buf),
                 boost::asio::buffer_size(buf));
  }

  auto fill(std::initializer_list<uint8_t> l) {
    return write(rBuf_, std::cbegin(l), l.size());
  }

  auto fill(ConstBuffer<uint8_t> buf) {
    return write(rBuf_, std::cbegin(buf), buf.size());
  }

  auto flush(MutableBuffer<uint8_t> buf) {
    return read(wBuf_, std::cbegin(buf), buf.size());
  }
...
public:
  explicit TestStream(TestSocket &socket, bool open = false)
      : socket_{socket}, open_{open} {}
  ...
  template <typename MutableBufferSequence>
  auto read_some(MutableBufferSequence const &buf) {
    assertTrue(is_open());
  ...
  template <typename ConstBufferSequence>
  auto write_some(ConstBufferSequence const &buf) {
    assertTrue(is_open());
...
auto asyncConnect(Stream &stream, ResolveResults results,
                  ConnectHandler &&handler) {
  ...
  return detail::asyncInitiate<void(boost::system::error_code)>(
      [](auto &&h, auto &stream, auto results) {
        ...
        static_assert(
            std::is_invocable_v<Handler, boost::system::error_code const &>);
        if (results.empty()) {
        ...
        stream.async_connect(*first,
                             detail::IteratorConnectOperator{stream, first, h});
      },
...
template <typename Stream, typename Iterator, typename ConnectHandler>
bool asio_handler_is_continuation(pichi::net::detail::IteratorConnectOperator<
                                  Stream, Iterator, ConnectHandler> *) {
  return true;
}

template <typename F, typename Stream, typename Iterator,
          typename ConnectHandler>
void asio_handler_invoke(F &&f, pichi::net::detail::IteratorConnectOperator<
                                    Stream, Iterator, ConnectHandler> *) {
  std::invoke(std::forward<F>(f));
}

template <typename Stream, typename Iterator, typename ConnectHandler>
struct associated_executor<pichi::net::detail::IteratorConnectOperator<
    Stream, Iterator, ConnectHandler>> {
  using type = executor;

  static type
  get(pichi::net::detail::IteratorConnectOperator<Stream, Iterator,
                                                  ConnectHandler> const &h,
