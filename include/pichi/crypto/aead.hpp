...
using AeadContext = std::conditional_t<
    detail::isGcm<method>(), mbedtls_gcm_context,
    std::conditional_t<
        detail::isSodiumAead<method>(), std::array<uint8_t, KEY_SIZE<method>>, void>>;
...
public:
  ...
  AeadEncryptor(AeadEncryptor&&) noexcept;

  AeadEncryptor& operator=(AeadEncryptor const&) = delete;
  AeadEncryptor& operator=(AeadEncryptor&&) noexcept;
...
public:
  ...
  AeadDecryptor(AeadDecryptor&&) noexcept;

  AeadDecryptor& operator=(AeadDecryptor const&) = delete;
  AeadDecryptor& operator=(AeadDecryptor&&) noexcept;
