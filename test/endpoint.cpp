...
BOOST_AUTO_TEST_CASE(serialize_Empty_Host)
{
  ...
  BOOST_CHECK_EXCEPTION(
      serializeEndpoint({EndpointType::IPV4, ""s, 1_u16}, buf),
      SystemError,
      verify_exception<PichiError::MISC>
  );
  BOOST_CHECK_EXCEPTION(
      serializeEndpoint({EndpointType::IPV6, ""s, 1_u16}, buf),
      SystemError,
      verify_exception<PichiError::MISC>
  );
}
...
BOOST_AUTO_TEST_CASE(serialize_Invalid_IP_Address)
{
  ...
  for (auto type : {EndpointType::IPV4, EndpointType::IPV6}) {
    BOOST_CHECK_EXCEPTION(
        serializeEndpoint({type, "localhost"s, 1_u16}, buf),
        SystemError,
        verify_exception<asio::error::invalid_argument>
    );
  }
}
...
BOOST_AUTO_TEST_CASE(parse_Invalid_Type)
{
  for (auto i = 0; i < 0x100; ++i) {
    ...
    stub.write(t);
    BOOST_CHECK_EXCEPTION(
        parseEndpoint([&stub](auto buf) { stub.read(buf); }),
        SystemError,
...
BOOST_AUTO_TEST_CASE(serializeEndpoint_Invalid_IP_Address)
{
  ...
  BOOST_CHECK_EXCEPTION(
      serializeEndpoint({EndpointType::IPV6, "1.1.1.1", 80_u16}, buf),
      SystemError,
      verify_exception<asio::error::invalid_argument>
  );
  BOOST_CHECK_EXCEPTION(
...
BOOST_AUTO_TEST_CASE_TEMPLATE(serializeEndpoint_Less_Buffer, Helper, Helpers)
{
  ...
  BOOST_CHECK_EXCEPTION(
      serializeEndpoint(Helper::ENDPOINT, {}),
      SystemError,
      verify_exception<PichiError::MISC>
  );
  BOOST_CHECK_EXCEPTION(
...
{
  BOOST_CHECK_EXCEPTION(detectHostType(""sv), SystemError, verify_exception<PichiError::MISC>);
