...
template <AdapterType type, typename Key> void verifyMandatoryField(Key&& key)
{
  ...
  BOOST_CHECK_EXCEPTION(parse<Ingress>(json), SystemError, verify_exception<PichiError::BAD_JSON>);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(parse_Optional_Fields, Trait, AllAdapterTraits)
{
  if constexpr (Trait::credential_ == Present::OPTIONAL) {
    ...
    BOOST_CHECK(
        parse<Ingress>(defaultIngressJson<Trait::type_>().AddMember(
            ingress::CREDENTIALS,
  ...
  if constexpr (Trait::option_ == Present::OPTIONAL) {
    ...
    BOOST_CHECK(
        parse<Ingress>(defaultIngressJson<Trait::type_>()
                           .AddMember(ingress::OPTION, defaultOptionJson<Option>(), alloc)) ==
        ingress
    );
  }
  if constexpr (Trait::tls_ == Present::OPTIONAL) {
    ...
    BOOST_CHECK(
        parse<Ingress>(defaultIngressJson<Trait::type_>().AddMember(
            ingress::TLS,
            defaultOptionJson<TlsIngressOption>(),
            alloc
        )) == ingress
    );
  }
...
BOOST_AUTO_TEST_CASE(toJson_Invalid_Adapter_Type)
{
  for (auto type : {AdapterType::DIRECT, AdapterType::REJECT}) {
    ...
    ingress.type_ = type;
    ingress.bind_.push_back(DEFAULT_ENDPOINT);
    BOOST_CHECK_EXCEPTION(toJson(ingress, alloc), SystemError, verify_exception<PichiError::MISC>);
  }
}
