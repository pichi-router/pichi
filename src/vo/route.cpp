...
json::Value toJson(Route const& rvo, Allocator& alloc)
{
  ...
  for_each(cbegin(rvo.rules_), cend(rvo.rules_), [&](auto&& item) {
    ...
    rule.SetArray();
    for_each(cbegin(item.first), cend(item.first), [&](auto&& name) {
      rule.PushBack(toJson(name, alloc), alloc);
    });
    rule.PushBack(toJson(item.second, alloc), alloc);
    rules.PushBack(std::move(rule), alloc);
  });
  route.AddMember(route::RULES, std::move(rules), alloc);
  return route;
}
...
template <> Route parse(json::Value const& v)
{
  ...
  if (v.HasMember(route::DEFAULT)) rvo.default_.emplace(parse<std::string>(v[route::DEFAULT]));
  ...
  parseArray(v, route::RULES, back_inserter(rvo.rules_), [](auto&& v) {
    ...
    return std::make_pair(
        std::accumulate(
            ...
            [](auto&& full, auto&& item) {
              full.push_back(parse<std::string>(item));
              return std::move(full);
            }
        ),
        parse<std::string>(*last)
    );
  });
...
{
  return lhs.default_ == rhs.default_ && rngs::equal(lhs.rules_, rhs.rules_);
}
