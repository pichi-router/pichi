...
conan_basic_setup(KEEP_RPATHS)""")
  ...
  def _configure_cmake(self):
    ...
    # Warning: if enabled, do not use cmake installation, to avoid installing files in OPENSSLDIR
    self._cmake.definitions["LIBRESSL_APPS"] = False
  ...
  def _lib_name(self, name):
    ...
    if self.settings.os == "Windows" and \
      ...
      lib_fullpath = glob.glob(os.path.join(
          self.package_folder, "lib", "*{}*".format(name)))[0]
      lib_name = os.path.basename(lib_fullpath).split(".")[
          0].replace("lib", "")
      return lib_name
    return name
